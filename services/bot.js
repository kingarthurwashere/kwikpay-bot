...
bot.on( "callback_query", async ( msg ) =>
{
  ...
  //HANDLE CHOICE OF PAYMENT METHODSaddAmount
  ...
  else if ( data == 'pesepayPayment' )
  {
    ...
    await bot.sendMessage( chatId, `Please enter the Amount in (USD):`, { reply_markup: { force_reply: true } } )
  }
  else if ( data == 'admin' )
  {
...
async function confirmPayment ( transaction, chatId )
{
  const exchangeRate = await currencyRateService.findByCurrencyFrom( 'USD' );
  if ( transaction.transactionType == 'airtime' )
  {
